...
public class GreetingController {
	...
	public GreetingController(GetUserInfoService getUserInfoService) {
		this.getUserInfoService = getUserInfoService;
	}

	@GetMapping("/greeting")
	@ResponseBody
	public Object greeting(@RequestParam(name="name", required=false, defaultValue="1") String name, Model model) {
...
//		getUserInfoService.getUserInfoById(name, model);
		...
		return getUserInfoService.getUserInfoById(name);
	}

	@PostMapping("/greetingPost")
	@ResponseBody
	public MVCMybatisDemoUser greetingPost(@RequestParam(name="name") String name) {
